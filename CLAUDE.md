...
### エンコード済み動画で字幕が「詰まる」(Issue #69 / 計測 PR #68)

再生中、映像・音声は正常なまま**字幕の更新だけが数秒〜数十秒止まる**ことがある。
30分番組で1回あるかないかと頻度が低く、同じ動画でも同じ位置では再現しない。
TS の ARIB 字幕では起きず、エンコード済み動画(tx3g 字幕)でのみ起きる。
リリースビルド・デバッグビルドの両方で発生する。

**症状が2通りに見えるが同一現象。** tx3g は「次のサンプルが来るまで現在の表示を維持する」
形式で、消去も“空サンプル”1件で行う。よって更新が止まると、表示中なら「字幕が出っぱなし」、
空サンプル表示中なら「しばらく字幕が出ない」になる。

**動画側は切り分け済み(原因ではない)。** サーバー上の録画 MP4 全169本について、moov を解析して
「各字幕サンプルがファイルのどのバイト位置にあるか」を実測した。ExoPlayer は MP4 をバイト順に
読むため、この値がそのまま「字幕を間に合わせるのに必要な先読み秒数」になる。
Amatsukaze(MP4Box/GPAC で mux)の出力165本は**すべて字幕が映像より前方**に書かれており、
必要先読みは最大0.5秒。字幕サンプルの供給が間に合わないことは構造的に起こり得ない。
ffprobe でも裏取り済み(字幕 pts 708秒がバイト29.1MB、映像はその位置でまだ130秒付近)。
字幕の表示継続時間も健全(実測: 中央値3.0秒 / p99 9.0秒、消去サンプルも正しく挿入されている)。

**常設計測コード:** `SubtitleStallDiagnostics.kt`。頻度が低くブランチを切り替えるたびに計測が
外れて取り逃すため、原因が判明するまで master に常駐させている。**原因確定後は丸ごと削除する**
(後片付けの手順は Issue #69 の残タスクに記載)。判定方法と正常時の実測値はクラスの KDoc を参照。
実機ログの採取手順は DEVELOPMENT.md「発生頻度の低い不具合を端末側ログで待ち伏せする」を参照。

**ここまでで判明していること(Google TV Streamer, 約4時間・Cue約2000件):**

- メインスレッド停止は0件。Cue の遅れは中央5ms・最大32ms、描画は最大18ms。通常再生は完全に正常
- 検出された遅延はすべて**シーク直後**で、これは ExoPlayer がシーク先で有効な字幕サンプルを
  復元する正常動作。計測コード側で `onPositionDiscontinuity` を見て除外済み
