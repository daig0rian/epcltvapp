...
起動して最初に表示されるホーム画面です。
ライブ視聴・録画中・最近の録画・番組表からの録画・検索履歴・録画ルールが縦に並びます。設定は画面上部の歯車ボタンから開きます。
左側のサイドバーにはセクションごとのアイコン付き見出しがあり、↑↓ で行を切り替え、→ または 決定 でその行の中に入ってカードを選べます。

### 1.1 初回起動時の設定

はじめて起動したときは、EPGStationがどこにあるかをアプリに教える必要があります。設定が済むまで録画一覧は空のままです。

**1. はじめての起動では設定画面が自動で開きます。接続設定 を選びます。**

閉じてしまったときは、画面上部の検索ボタンの右隣にある歯車ボタンから開き直せます（サイドバーを開いているときに出ます）。

![設定画面の接続設定を選ぶ](screenshots/setup-1-settings-screen.png)
...
![番組表からの録画の行](screenshots/row-manual-recorded.png)

録画ルールを使わずに、番組表から直接予約して録画した番組が並びます。EPGStationは録画のひとつひとつに予約元のルールを記録しているので、その記録を持たない録画をこの行に集めています。こうした録画はルール別の行のどこにも現れないため、まとめて見に行ける場所として用意しています。

該当する録画が1件もないときも行は残ります。集める範囲は新しい録画3000件までなので、それより古いものはこの行に出てこないことがあります。
...
EPGStationに登録済みの自動録画ルールごとに1行並びます。キーワード未設定のルールは「ルールID ○○」と表示されます。

並び順は設定画面から3つのうちどれかを選べます。**「録画の新しい順」を選ぶと、最近録画があったルールほど上に来ます**ので、いま追いかけている番組のルールが自然と手前に集まります。しばらく録画のないルールと、録画が1件もないルールは末尾に回ります。録画0件のルールを隠すかどうかも設定で切り替えられます。

### 1.3 設定を開く

![タイトル行の歯車ボタン](screenshots/settings-entry-gear.png)

設定は、画面上部の検索ボタンの右隣にある**歯車ボタン**から開きます。サイドバーを開いているときに現れ、サイドバーの一番上の行で ↑ を押すと検索ボタンへ移り、そこから → で歯車へ移れます。

設定画面には「接続設定」「再生設定」「表示設定」「録画の再読み込み」「アップデートを確認」が並びます。前の3枚はそれぞれの設定画面へ入ります。「録画の再読み込み」を選ぶと録画一覧が最新の状態になります。「アップデートを確認」はアプリ自身の更新に使います（7章参照）。
...
>
> - **CC(字幕)** … 字幕が含まれているときに出ます。放送そのままのTS（「TSを再生」「簡易追いかけ再生(TS)」「ライブ視聴（単押し）」）では、設定画面の「再生設定 → TS処理 → ネイティブTS処理」がONのときだけ字幕を扱えます。**初期状態はON**です。エンコード済み動画では、その動画に字幕が入っていれば設定に関わらず出ます。
...
画面の右側にせり出す形で表示されます。画面上部の歯車ボタンから開き、「接続設定」「再生設定」「表示設定」の3つに分かれています。あわせて「録画の再読み込み」と「アップデートを確認」も同じ画面から実行できます。
...
- **ネイティブTS処理**: 字幕・副音声の切り替え、文字スーパーの表示、TSの安定化処理を使うかどうかのスイッチです。**初期状態はON**で、内蔵プレーヤーがTSを安定的に再生できるようになるほか、CC・音声のボタンが出ます（4.3節参照）。以前のバージョンから使い続けている端末では、そのとき入っていた設定がそのまま残ります（以前の初期状態はOFFでした）。

> **一部の機種での報告**: ONのときにアプリが終了する、OSが再起動する、という報告があります。原因は特定できておらず、すべての機種で起きるわけでもありません。
>
> こうした症状が出た場合はOFFに戻してください。OFFのあいだは、放送そのままのTSを再生するときに字幕・副音声の切り替えと文字スーパーの表示が使えなくなります（エンコード済み動画には影響しません）。

### 5.3 表示設定

![表示設定](screenshots/settings-display.png)

- **全体設定**: サムネイル背景表示のON/OFFを切り替えます。
- **ルール表示設定**: 録画ルールの並び順を選べます。**初期状態は「ルールの新しい順」**です。

  | 選択肢 | 並び |
  |---|---|
  | ルールの新しい順 | あとから登録したルールほど上 |
  | ルールの古い順 | 先に登録したルールほど上 |
  | 録画の新しい順 | そのルールで最後に録画された番組が新しいものほど上。録画が1件もないルールは末尾 |

  録画が0件の録画ルールを一覧に表示するかどうかも、ここで切り替えます。
- **履歴表示設定**: 検索履歴を新しい順に並べるかどうかと、表示件数を選べます（0/1/2/3/5/8/13/21件）。検索履歴を今すぐ全部消すボタンもあります。
...
このアプリはストアで配布していないため、更新は自分で行う必要があります。設定画面の **アップデートを確認** から、新しいバージョンの確認・ダウンロード・インストールまでをアプリの中で済ませられます。

![アップデートを確認する前の確認画面](screenshots/update-consent.png)

カードを選ぶと、まず **GitHub へ接続してよいか** を尋ねる画面が出ます。ここで「接続して確認」を選んだときにだけ、アプリはインターネットへ出て最新リリースを調べにいきます。初期状態でカーソルが当たっているのは「キャンセル」なので、決定ボタンを押し間違えても外へは出ていきません。

> **このアプリが自分からインターネットへ出ることはありません。** 起動時のチェックも、バックグラウンドでの定期確認もしていません。確認した結果を端末に残すこともしないので、閉じればいつでも元どおりで、カードの見た目が新しいバージョンの有無で変わることもありません。
>
> 裏を返すと、**新しいバージョンが出たことに自動で気づく仕組みはありません。** ときどき自分で確認してください。

### 7.1 最新だったとき

![最新だったときの表示](screenshots/update-up-to-date.png)

現在使っているバージョンが表示されます。「閉じる」で終わりです。

### 7.2 新しいバージョンがあったとき

リリースノートが表示されます。長いときは↑↓でスクロールできます。ここでも初期状態のカーソルは「あとで」に当たっているので、読むだけで閉じても構いません。

「更新する」を選ぶとAPKのダウンロードが始まり、進み具合が％で表示されます。「中止」でいつでも止められます。

ダウンロードが終わると、端末のインストール確認画面に切り替わります。あとは画面の案内に従ってください。インストールが完了するとアプリはいったん終了するので、もう一度起動してください。

### 7.3 「不明なアプリのインストール」の許可

ストア以外から入れたAPKをインストールするには、端末側でこのアプリに許可を与える必要があります（Android 8.0以降）。許可がない場合はその旨が表示されるので、「設定を開く」から許可してください。許可したあとは自動的にインストールへ進みます。

機種によってはこの設定画面をアプリから直接開けないことがあります。その場合は手順が文章で表示されるので、端末の設定から手動でたどってください。

### 7.4 うまくいかないときに出るメッセージ

| 表示 | 意味 |
|---|---|
| GitHub に接続できませんでした。… | インターネットに出られていません。ネットワークの状態を確認してください |
| リリース情報を読み取れませんでした。 | GitHubからの応答を解釈できませんでした。時間をおいて「再試行」してください |
| 最新のリリースに APK が添付されていません。 | 配布ファイルが見つかりません。[Releases](https://github.com/daig0rian/epcltvapp/releases) から手動で入れてください |
| ダウンロードに失敗しました。 | 途中で通信が途切れました。「再試行」でやり直せます |
| ダウンロードに使える保存領域がありません。 | 端末の空き容量を確認してください |
| インストーラーを起動できませんでした。 | 端末側でAPKのインストールが行えない状態です。[Releases](https://github.com/daig0rian/epcltvapp/releases) から手動で入れてください |
