...
| 速度計 | 速度変更 | 押すとボタンの上に **等速 / 1.25倍 / 1.5倍 / 2倍** の一覧が開きます。上下キーで選び、もう一度決定を押すと確定します（戻るキーで、変えずにやめられます）。音の高さは変わりません。等速以外で再生している間はアイコンが水色になります。ライブ視聴では表示されません。 |

アイコンの色で今の状態がわかります。**白は OFF / 主音声 / 等速**、**水色は ON / 副音声 / 等速以外** です。RECだけは中央が赤い丸で、押すと「REC...」の文字に変わります。
...
>
> シームレス追いかけ再生(HLS)では、CC・SI・音声のボタンは表示されません。
...
**再生速度だけは記憶されません。** 再生を始めるたびに等速に戻ります（シリーズの次の回へ自動で進んだときも同じです）。前に見た番組で2倍速にしたことを忘れたまま次の番組が速く始まる、ということが起きないようにするためです。
